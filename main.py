...
def call_result(n1, n2):
    ...
    consumerKey = "jWaXkDYH56eaMTIKlPWe5kHBu"
    consumerSecret = "CYY47YoVzuufFWCVe54oxklesrN1A9qUzxCL8qkgJGcx7OgV6Z"
    accessToken = "886236788850905088-5Xg5vChIWwQPxTNV8DKefeBCErhdHht"
    accessTokenSecret = "PTu0bJFFoO6iphZ6tQ5RuZpj59gWzFjYnGU2anza8av3L"
...
labelTitle = tk.Label(root, text=" Twitter Sentiment Analysis").grid(row=0, column=1)
labelNum1 = tk.Label(root, text="Enter Hashtag to search :").grid(row=1, column=0)
labelNum2 = tk.Label(root, text="Number of Tweets to examine :").grid(row=2, column=0)
...
buttonCal = tk.Button(root, text="Generate the Result", command=call_result,width="21").grid(row=3, column=1,pady=10)

button1 = tk.Button(root, text="View Positive Tweets", command=call_readcsvP,width="21").grid(row=5, column=0,padx=15)
button2 = tk.Button(root, text="View Negative Tweets", command=call_readcsvNeg,width="21").grid(row=5, column=1,pady=45)
button3 = tk.Button(root, text="View Neutral Tweets", command=call_readcsvN,width="21").grid(row=5, column=2,padx=15)
root.mainloop()
